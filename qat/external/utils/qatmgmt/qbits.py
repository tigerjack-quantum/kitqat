...
def get_qbits_to_int_mapping_from_qregs(qregs: List["QRegister"]) -> Dict[int, "Qbit"]:
    qregs_flat_dict = {
        qbit: qbit.index
        for qbit in functools.reduce(
            operator.concat, map(operator.attrgetter("qbits"), qregs)
        )
    }
...
def add_name_to_qbits_following_pattern(
    program: "Program", pattern: Dict[str, List["Qbit"]]
):
